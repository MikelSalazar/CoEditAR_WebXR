/******************************************************************************
 CoEditAR Build System - Codebase
******************************************************************************/
	...
	for (itemIndex = 0; itemIndex < itemCount; itemIndex++) {
		...
		if (classType !== null && item.level == 1) {
			...
			if (item.type == ItemTypes.word || isConstructor) {
				...
				// To avoid problems with special members like "toString"
				if (!member) {
					debugger
				}
 ...
 *  @param classOrder The initial class order (to fix order issues). */
function reorderFilePaths(initialFilepaths = [], classOrder = []) {
	...
	// Reorder the list of file paths, using the class hierarchy
	for (classIndex = 0; classIndex < classCount; classIndex++) {
		console.log(classOrder[classIndex]);
	...
	// Return the resulting list of filepaths
	return filePaths;
}
...
function build() {
	...
	filePaths = reorderFilePaths([], []); // Insert here the classes
}
