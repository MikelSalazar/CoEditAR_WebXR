import { Class, Domain, Metadata, Property, Serialization } from "../../CoEditAR";
...
export class Instance {
	...
	constructor (classEx?: Class | string, data?: any) { 
		...
		if (typeof classEx == "string") {
			...
			else throw Error('Unknown class "' + classEx + '"');
		}
		for (let c of classes) {
			if (!c || !c.isClass) throw Error ("Invalid class to instantiate");
			this._metadata.classes.push(c);
		}
		
		// Instantiate the classes (adding the properties to the class)
		for (let c of this._metadata.classes) c.instantiate(this);
	...
	deserialize(data: any) {
		...
		for (let requiredProperty of this.metadata.required) 
			if ((!data || !data[requiredProperty])) throw new Error(
				'Required property not defined: "' + requiredProperty + '"');

		function createProperty(instance: any, name: string, property: Property) {
	...
	* @return The serialized data. */
	serialize(params?: any, data: any = {}): any {
		...
		for (let propertyName in this.metadata.properties) {
			...
			else data[propertyName] = propertyData;
		}
		
		// If there are serialization parameters language, use them
		if (params) return Serialization.serialize(params, data);
	 ...
	 * relations or not.*/
	destroy(propagate: boolean = true) {}
}
