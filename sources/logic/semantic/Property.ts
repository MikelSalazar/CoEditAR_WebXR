import { Class, Instance } from "../../CoEditAR";
...
export class Property extends Serializable {
	...
	static formats: string[] = [
		"date-time",				// Date and time (yyyy-MM-ddThh:mm:ss)
		"time",						// Time (hh:mm:ss)
		"date",						// Date (yyyy-MM-dd)
		"duration",					// Time duration (ISO 8601)
		"email",					// Email address (RFC 5321, 4.1.2)
		"idn-email",				// Internationalized email address(RFC 6531)
		"hostname",					// Internet host name (RFC 1123, 2.1)
		"idn-hostname",				// Internationalized host address (RFC 6531)
		"ipv4",						// IPv4 address (RFC 2673, 3.2)
		"ipv6",						// IPv6 address (RFC 2373, 2.2)
		"uuid",						// Universally Unique Identifier (RFC 4122)
		"uri",						// Universal Resource Identifier (RFC 3986)
		"uri-reference",			// URI Reference (RFC 3986, section 4.1) 
		"iri",						// Internationalized URI (RFC 3987)
		"iri-reference",			// Internationalized URI reference RFC 3987)
		"uri-template",				// URI template (RFC 6570)
		"json-pointer",				// JSON pointer (RFC 6901)
		"relative-json-pointer",	// Relative JSON pointer (RFC 6901)
	...
	private _original: Property;

	/** The copied instances of the property (if not unique). */
	private _copies: Property[];
	...
	set type(type: string) {
		if (Property.types.includes(type)) this._type = type;
	...
	set unique(unique: boolean) { this._unique = unique; }
	...
	set value(newValue: any) {
		if (this.checkValue(newValue, true)) this._value = newValue;
	}

	/** The default value of the property. */
	get default(): any { return this._default; }
	set default(defaultValue: any) { 
		if (this.checkValue(defaultValue, true)) this._default = defaultValue; 
	}

	/** The valid values of the property. */
	get enum(): any[] { return this._enum; }
	set enum(ev: any[]) { 
	...
	/** The minimum number of items contained in an array property. */
	get minItems(): number { return this._minItems; }
	...
	/** The original property instance (if not unique). */
	get original(): Property { return this._original; }

	/** The copied instances of the property (if not unique). */
	get copies(): Property[] { return this._copies; }
	...
	constructor(name: string, data?: any) {
		...
		if (!this._name) throw Error('No name provided for Property');
		...
		this._description = new LocalizedString(
			'PropertyDescription' + this._name); 

		// Initialize the different fields
		this._type = 'object'; // By default every property defines an object
		this._required = false; this._unique = false; this._readOnly = false;
		this._classes = []; this._instances = []; 
		this._original = null; this._copies = [];
		this._value = undefined; this._default = undefined; 
		this._enum = undefined;
		this._minimum = undefined; this._maximum = undefined;
		this._minLength = undefined; this._maxLength = undefined;
		this._pattern = undefined; this._format = undefined;
		this._items = []; 
		this._minItems = undefined; this._maxItems = undefined;
	...
	clone(): Property {
		...
		this._copies.push(copy); copy._original = this.original || this;
	...
	deserialize(data: any): void {
		...
		if (data.required != undefined) this.required = data.required == true;
		if (data.unique != undefined) this.unique = data.unique == true;
		if (data.readonly != undefined) this.readOnly = data.readonly == true;
		...
		if (data.minItems != undefined) this.minItems = data.minItems;
		if (data.maxItems != undefined) this.maxItems = data.maxItems;
		...
		if (data.items != undefined) {
			...
			for(let itemIndex = 0; itemIndex < data.items.length; itemIndex++)
				this._items.push(new Property(this.name + itemIndex, 
		...
		this._classes = [];
		if (data.classes) for (let c of data.classes) {
		...
		this._instances = [];
		if (data.instances) for (let i of data.instances) this.associate(i);

		// Create the link between original and the copies
		this._original = data.original; this._copies = [];
		if (data.copies) for (let c of data.copies) this._copies.push(c);

		// If undefined, set the default values depending on the type
		if (this._default == undefined) {
			switch (this._type) {
				case "null": this._default = null; break;
				case "boolean": this._default = false; break;
			}
		}
	}
	...
	* @return The resulting JSON (schema) data. */
	serialize(params?: any, data: any = {}): any {
		...
		if (!this._title.isNull) data.title = this._title.serialize();
		if (!this._description.isNull)
			data.description = this._description.serialize();
		if (this._type !== "object") data.type = this._type;
		if (this._required != false) data.required = this._required;
		if (this._unique != false) data.unique = this._unique;
		if (this._readOnly != false) data.readOnly = this._readOnly;
		if (this._value != undefined) data.value = this._value;
		if (this._default != undefined) data.default = this._default;
		if (this._enum != undefined) data.enum = this._enum;
		if (this._minimum != undefined) data.minimum = this._minimum;
		if (this._maximum != undefined) data.maximum = this._maximum;
		if (this._minLength != undefined) data.minLength = this._minLength;
		if (this._maxLength != undefined) data.maxLength = this._maxLength;
		if (this._pattern != undefined) data.pattern = this._pattern;
		if (this._format != undefined) data.format = this._format;
		if (this._minItems != undefined) data.minItems = this._minItems;
		if (this._maxItems != undefined) data.maxItems = this._maxItems;

		// Serialize the sub-properties
		if (this._items != undefined && this._items.length > 0) {
			data.items = []; 
			for (let item of this._items) {
				let itemData = item.serialize(); itemData.name = undefined;
				data.items.push(itemData);
			}
		}

		// If there are serialization parameters language, use them
		if (params) return Serialization.serialize(data, params);
	...
	associate(instances: Instance[] | Instance) {

		// Check the provided
		if (!instances) throw Error("No instances provided");
		if (!Array.isArray(instances)) instances = [instances];
		for (let instance of instances) {

			// If it is unique, just link this property
			let properties = instance.metadata.properties;
			if (this._unique) properties[this._name] = this;

			// Otherwise, create a copy of this property
			else properties[this._name] = this.clone();
	...
	checkValue(value: any, throwExceptions: boolean = false): boolean {
		...
		if (this._readOnly) throw Error('Property "' + this.name + '" ' +
			'cannot receive a value because is marked as "read only"');

		// If there is a enumeration, check the values
		if (this._enum) {
			if (!this._enum.includes(value)) throw Error(errorMessage(
				'valid options are "' + this.enum.join('", "') +  '"'));
		}
		...
		switch (this._type) {
			case 'null':
				...
				throw Error(errorMessage(
					'A null property can not have a value'));
			case 'boolean':
			...
			case 'integer': case 'number':
				...
				if (this._minimum != undefined && value < this._minimum) 
					if (!throwExceptions) return false
						...
						'the minimum value is ' + this._minimum));
				if (this._maximum != undefined && value > this._maximum) 
					if (!throwExceptions) return false
					else throw Error(errorMessage(
						'the maximum value is '+ this._maximum));
					break;
			case 'string':
				if (typeof value != "string") throw Error(errorMessage(
					'invalid value type "' + typeof value + '"'));
				if (this._minLength && value.length < this._minLength) 
					if (!throwExceptions) return false
					else throw Error(errorMessage(
						'the minimum text size is ' + this._minLength));
				if (this._maxLength && value.length > this._maxLength) 
					if (!throwExceptions) return false
					else throw Error(errorMessage(
						'the maximum text size is '+ this._maxLength));
				// TODO Check pattern and format
			break;
			case 'array': {
				if (!Array.isArray(value)) throw Error(errorMessage(
				...
				if (this._items && this._items.length > 0) {
					for (let itemValue of value) {
						...
						if (!validItem) if (!throwExceptions) return false;
							else throw Error(errorMessage('the item "' + 
				...
				if (this._minItems && value.length < this._minItems) 
					if (!throwExceptions) return false
						...
						'the minimum number of items is ' + this._minItems));
				if (this._maxItems && value.length > this._maxItems) 
					if (!throwExceptions) return false
					else throw Error(errorMessage(
						'the maximum number of items is ' + this._maxItems));
			}
		}
